...
export var fingerprint = {
  ...
  "width": 1792,
  "colorDepth": 30,
  "locationHref": "https://mms.pinduoduo.com/login?redirectUrl=https%3A%2F%2Fmms.pinduoduo.com%2Fhome",
  "clientWidth": 1741,
  "clientHeight": 883,
  "offsetWidth": 1741,
  "offsetHeight": 883,
...
function sign(r) {
        ...
        try {
          ...
          for (t = 0, o = r.length; t < o; t++)
            (n = r.charCodeAt(t)) < 128
              ...
              : n < 55296 || n >= 57344
              ? _.push(224 | (n >> 12), 128 | ((n >> 6) & 63), 128 | (63 & n))
              : (t++,
  ...
  /**
   * hack
   * @type {{locationHref: string, referer: string, innerHeight: number, scrollHeight: number, navigator: {product: string, cookieEnabled: boolean, appName: string, plugins: string, language: string, appCodeName: string, ua: string, productSub: string, platform: string, hardwareConcurrency: number, doNotTrack: string, mimeTypes: string, vendor: string}, offsetWidth: number, clientHeight: number, devicePixelRatio: number, innerWidth: number, availWidth: number, clientWidth: number, timezoneOffset: number, offsetHeight: number, width: number, colorDepth: number, availHeight: number, height: number, scrollWidth: number}}
   */
  // var f = this;
  var f = fingerprint;
  switch (
    ((f = f[t + n + "m" + A + "ntB" + ["d", "I", "y"].reverse().join("")]),
    "object" !== typeof o && (o = {}),
    o.encoding)
  ) {
    case "hex":
  ...
  for (O = 0; O < d; O++) {
    ...
    for (y = e._bæ; y < 64; y++)
      (s[y] = e._ct(s[y - 2]) + s[y - 7] + e._dY(s[y - 15]) + s[y - 16]),
        (s[y] &= 4294967295);
    for (
      v = g[0],
        w = g[1],
...
function genRiskSign(username, encryptedPassword, timestamp){
  const s = `username=${username}&password=${encryptedPassword}&ts=${timestamp}`;
  return sign(s);
};

export default genRiskSign;

if (require.main === module) {
  // var signFrom =
  //   "username=乐和食品店:冯露&password=iTT60Clq7KEqGnkeZxUpC/z274wEtQSNhPPp0oDdLqYDXlMeDTmQUGt/YDyE5/BPrIgHMqSZkGrpM+yd6oXhrsEC8wble1X8ofLypo/PMpsZP5vSJy1x6WQsvrE3+dpdPcr7zzXHGCj0fvgko2V9Caby5Oo6uHeNT/b1ljAOcho=&ts=1619873822047";
  // s = sign(signFrom);
  // assert(
  //   s === "b20217c27d4cda17918863c63d26952f7141000740fc568468d4df4a4520ad5d"
  // );
  var r = "乐和食品店:冯露";
  var t =
    "Me/he1+ymUBa902628aXQu6t+dWvWHC+bpWQBx3WkZhzipIcXdWvQCzdDELwKrjSmaE1I7E8Dtt0RkywJGUJlrd9hbesAgFldxK+Qc7rboPjgVBjaPS3IetODt3GshuVRK7knKMMPignTu6Z2DkDnictdsbsQtiYC6iu/TeMcg4=";
  var n = 1619887674010;
  assert(
    hackSign(r, t, n) ===
      "3eec343528c0bf2ed9b5a15d4596223a4b20f7bb7e7b13e837ba85f027218b3d"
  );
  console.log("yes");
}
